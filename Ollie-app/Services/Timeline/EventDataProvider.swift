...
final class EventDataProvider {
    ...
    private func refreshHistoricalData() async {
        ...
        // SYNC FIX: Deduplicate events by ID to handle sync edge cases where
        // the same event might exist in both private and shared stores
        _monthEvents = deduplicateAndSort(monthHistorical + todayFiltered)
        _extendedEvents = deduplicateAndSort(extendedHistorical + todayFiltered)
        _eightDayEvents = deduplicateAndSort(eightDayHistorical + todayFiltered)
        _weekEvents = deduplicateAndSort(weekHistorical + todayFiltered)
        _recentEvents = deduplicateAndSort(yesterdayHistorical + todayFiltered)
        sortSpan?.finish()
    ...
    /// Deduplicate events by ID (keeping first occurrence) and sort by time descending
    private func deduplicateAndSort(_ events: [PuppyEvent]) -> [PuppyEvent] {
