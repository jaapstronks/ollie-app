...
final class ProfilePhotoStore: @unchecked Sendable {
    ...
    func loadAsync(filename: String, profileId: UUID) async -> UIImage? {
        let url = directory.appendingPathComponent(filename)
        ...
        do {
            let success = try await downloadFromCloudKit(profileId: profileId, to: url)
            if success {
    ...
    @MainActor
    private func downloadFromCloudKit(profileId: UUID, to destinationURL: URL) async throws -> Bool {
        try await CloudKitService.shared.downloadProfilePhoto(
            profileId: profileId,
            to: destinationURL
        )
    }
    ...
    func syncFromCloudKitIfNeeded(filename: String, profileId: UUID) async {
        guard !exists(filename: filename) else { return }

        let destinationURL = fullPath(for: filename)
        do {
            let success = try await downloadFromCloudKit(profileId: profileId, to: destinationURL)
            if success {
