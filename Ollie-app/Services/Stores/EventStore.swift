...
/// Architecture: Core Data with CKSyncEngine for CloudKit sync (coordinated via SyncCoordinator)
@Observable
@MainActor
class EventStore {
    ...
    func handleFetchedResultsChange() {
        ...
        // SYNC FIX: Invalidate range caches BEFORE posting notification
        // This ensures sleep state calculations get fresh data when FRC fires
        // during CloudKit sync (before the debounced handleRemoteChange callback)
    ...
    func refreshFromCloud() async {
        // CloudKit sync is handled by SyncCoordinator/CKSyncEngine
    ...
    func addEvent(_ event: PuppyEvent, profile: PuppyProfile? = nil) {
        ...
        do {
            ...
            let recordID = SyncCoordinator.shared.recordID(for: "CDPuppyEvent", id: newEvent.id)
    ...
    func deleteEvent(_ event: PuppyEvent, profile: PuppyProfile? = nil) {
        logger.info("Deleting event: id=\(event.id), type=\(event.type.rawValue), time=\(event.time), hasPhoto=\(event.photo != nil)")
        ...
        if event.cloudPhotoSynced == true {
            logger.debug("Deleting cloud photo for event \(event.id)")
            Task {
        ...
        // This also adds a tombstone to prevent resurrection from stale CloudKit records
        let recordID = SyncCoordinator.shared.recordID(for: "CDPuppyEvent", id: event.id)
        SyncCoordinator.shared.markPendingDelete(recordID: recordID)
        logger.debug("Queued CloudKit deletion with tombstone for event \(event.id)")

        do {
            try coreDataStore.deleteEvent(event)
            logger.info("Successfully deleted event from Core Data: \(event.id)")
    ...
    func updateEvent(_ event: PuppyEvent, profile: PuppyProfile? = nil) {
        ...
        do {
            ...
            // Queue for CloudKit sync
            let recordID = SyncCoordinator.shared.recordID(for: "CDPuppyEvent", id: updatedEvent.id)
            SyncCoordinator.shared.markPendingSave(recordID: recordID)
        ...
        guard var event = events.first(where: { $0.id == eventId }) else {
            ...
            if let storedEvent = coreDataStore.fetchEvent(byId: eventId) {
                ...
                do {
                    ...
                    // Queue for CloudKit sync
                    let recordID = SyncCoordinator.shared.recordID(for: "CDPuppyEvent", id: eventId)
                    SyncCoordinator.shared.markPendingSave(recordID: recordID)
    ...
    /// Mark a photo as not synced to CloudKit (photo doesn't exist in CloudKit)
    /// Called when download fails because the EventMedia record doesn't exist
    /// This resets the cloudPhotoSynced flag so the UI shows the correct state
    func markPhotoAsNotSynced(eventId: UUID) {
        // Try in-memory first
        if var event = events.first(where: { $0.id == eventId }) {
            // Only update if currently marked as synced
            guard event.cloudPhotoSynced == true else { return }

            event.cloudPhotoSynced = false
            event.cloudPhotoOwner = nil
            do {
                try coreDataStore.saveEvent(event)
                // Queue for CloudKit sync to update the event record
                let recordID = SyncCoordinator.shared.recordID(for: "CDPuppyEvent", id: eventId)
                SyncCoordinator.shared.markPendingSave(recordID: recordID)
                if let index = events.firstIndex(where: { $0.id == eventId }) {
                    events[index] = event
                }
                logger.info("Marked photo as NOT synced for event \(eventId) - photo not found in CloudKit")
            } catch {
                logger.error("Failed to mark photo as not synced: \(error.localizedDescription)")
            }
            return
        }
        ...
        // Event might be on a different day, fetch from store
        if let storedEvent = coreDataStore.fetchEvent(byId: eventId) {
            guard storedEvent.cloudPhotoSynced == true else { return }

            var mutableEvent = storedEvent
            mutableEvent.cloudPhotoSynced = false
            mutableEvent.cloudPhotoOwner = nil
            do {
                try coreDataStore.saveEvent(mutableEvent)
                // Queue for CloudKit sync to update the event record
                let recordID = SyncCoordinator.shared.recordID(for: "CDPuppyEvent", id: eventId)
    ...
    func forceSync() async {
        // Sync is coordinated by SyncCoordinator/CKSyncEngine
    ...
    func retryPendingOperations() async {
        // Retries are handled by SyncCoordinator/CKSyncEngine
    }
