...
//  Otis-app
//
//  Manages training plan data and skill progress tracking with Core Data and CKSyncEngine-based CloudKit sync
//
...
final class TrainingPlanStore: BaseStore {
    ...
    override var cloudKitRecordType: String? { "CD_CDMasteredSkill" }
    ...
    func markAsMastered(_ skillId: String) {
        ...
        performSave(operation: "Marked skill as mastered: \(skillId)", entityId: skill.id) {
    ...
    func unmarkMastered(_ skillId: String) {
        ...
        if let cdSkill = CDMasteredSkill.fetch(bySkillId: skillId, in: viewContext) {
            ...
            performDelete(operation: "Unmarked skill as mastered: \(skillId)", entityId: skill.id) {
