...
//  Otis-app
//
//  Manages medication completion tracking with Core Data and CKSyncEngine-based CloudKit sync
//
...
final class MedicationStore: BaseStore {
    ...
    override var cloudKitRecordType: String? { "CD_CDMedicationCompletion" }
    ...
    func markComplete(medicationId: UUID, timeId: UUID, for date: Date) -> MedicationCompletion {
        ...
        performSave(operation: "Marked medication \(medicationId) time \(timeId) as complete", entityId: completion.id) {
    ...
    func deleteCompletion(_ completion: MedicationCompletion) {
        ...
        performDelete(operation: "Deleted medication completion \(completion.id)", entityId: completion.id) {
