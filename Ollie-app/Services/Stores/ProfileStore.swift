...
class ProfileStore {
    ...
    func saveProfile(_ newProfile: PuppyProfile) {
        ...
        do {
            ...
            let recordID = SyncCoordinator.shared.recordID(for: "CDPuppyProfile", id: updatedProfile.id)
    ...
    func deleteProfile(_ profileId: UUID) {
        ...
        let recordID = SyncCoordinator.shared.recordID(for: "CDPuppyProfile", id: profileId)
    ...
    func loadAllProfiles() {
        ...
        var loadedProfileIds = Set<UUID>()

        // DEBUG: Direct entity count to verify data exists
        let debugRequest = NSFetchRequest<NSManagedObject>(entityName: "CDPuppyProfile")
        let debugCount = (try? viewContext.count(for: debugRequest)) ?? -1
        logger.info("DEBUG: Total CDPuppyProfile entities in store: \(debugCount)")

        // Also check what entities exist in the model
        if let entityNames = viewContext.persistentStoreCoordinator?.managedObjectModel.entities.map({ $0.name ?? "?" }) {
            logger.info("DEBUG: Available entities: \(entityNames.joined(separator: ", "))")
        }
        ...
        if let privateStore = persistenceController.getPrivateStore() {
            ...
            logger.info("DEBUG: fetchAllProfiles from privateStore returned \(ownedCDProfiles.count) CDPuppyProfile objects")
            let owned = ownedCDProfiles.compactMap { cdProfile -> PuppyProfile? in
                let result = cdProfile.toPuppyProfile(ownership: .owned)
                if result == nil {
                    logger.warning("DEBUG: toPuppyProfile returned nil for CDPuppyProfile id=\(cdProfile.id?.uuidString ?? "nil"), name=\(cdProfile.name ?? "nil")")
                }
                return result
            }
        ...
        // Migration: Also check for profiles not assigned to a specific store
        // This handles profiles created before the dual-store CKSyncEngine migration
        let allCDProfiles = CDPuppyProfile.fetchAllProfiles(in: viewContext)
        let unassignedProfiles = allCDProfiles.compactMap { cdProfile -> PuppyProfile? in
            guard let id = cdProfile.id, !loadedProfileIds.contains(id) else {
                return nil // Already loaded from a specific store
            }
            logger.info("Found unassigned profile (pre-migration): \(cdProfile.name ?? "unknown")")
            return cdProfile.toPuppyProfile(ownership: .owned)
        }
        if !unassignedProfiles.isEmpty {
            allProfiles.append(contentsOf: unassignedProfiles)
            logger.info("Migrated \(unassignedProfiles.count) profile(s) from pre-CKSyncEngine storage")
        }
